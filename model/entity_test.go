...
func TestRequestCloneAndTags(t *testing.T) {
	original := Request{ID: "one", Name: "First", Tags: []string{"Blue"}, Metadata: map[string]string{"zone": "a"}}
...
func TestRequestValidationAndTouch(t *testing.T) {
	if err := (Request{}).Validate(); err == nil {
		t.Fatal("expected validation error")
	}
	now := time.Unix(100, 0).UTC()
	value := (Request{ID: "one", Name: "First"}).Touch(now)
