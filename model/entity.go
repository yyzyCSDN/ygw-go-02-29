...
// Request is the canonical record managed by Request Gateway.
type Request struct {
	ID        string            `json:"id"`
...
func (value Request) Validate() error {
	...
	if strings.TrimSpace(value.Name) == "" {
		return fmt.Errorf("request name is empty")
	}
	if value.Priority < 0 {
		return fmt.Errorf("request priority cannot be negative")
	}
	if value.Amount < 0 {
		return fmt.Errorf("request amount cannot be negative")
	}
	for key := range value.Metadata {
		if strings.TrimSpace(key) == "" {
			return fmt.Errorf("request metadata contains a blank key")
		}
	}
	return nil
}

func (value Request) HasTag(tag string) bool {
...
func (value Request) MetadataValue(key string) (string, bool) {
	result, ok := value.Metadata[key]
	return result, ok
}

func (value Request) WithTag(tag string) Request {
	copy := value.Clone()
	tag = normalizeToken(tag)
...
func (value Request) WithoutTag(tag string) Request {
	copy := value.Clone()
	tag = normalizeToken(tag)
...
func (value Request) WithMetadata(key, content string) Request {
	copy := value.Clone()
...
func (value Request) Touch(now time.Time) Request {
	copy := value.Clone()
