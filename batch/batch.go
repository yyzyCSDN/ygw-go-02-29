...
type Result struct {
	Values  []model.Request
	Created int
	Updated int
	Deleted int
}

func Apply(current []model.Request, operations []Operation) (Result, error) {
	working := map[string]model.Request{}
	...
	for index, operation := range operations {
		...
		case Delete:
			if _, exists := working[operation.ID]; !exists {
				return Result{}, fmt.Errorf("operation %d: request %q does not exist", index+1, operation.ID)
			}
	...
	sort.Strings(ids)
	result.Values = make([]model.Request, 0, len(ids))
	for _, id := range ids {
...
func ValidateUnique(values []model.Request) error {
