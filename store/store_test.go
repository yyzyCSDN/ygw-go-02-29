...
func sample(id, name string) model.Request {
	return model.Request{ID: id, Name: name, Active: true, Tags: []string{"sample"}}
}
...
func TestStoreReplaceAllIsAtomic(t *testing.T) {
	...
	err := values.ReplaceAll([]model.Request{sample("same", "One"), sample("same", "Two")})
	if err == nil {
		t.Fatal("expected duplicate error")
	}
	...
	if err := values.ReplaceAll([]model.Request{sample("new", "New")}); err != nil {
		t.Fatal(err)
	}
