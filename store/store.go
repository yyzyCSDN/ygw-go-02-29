...
func New(historyLimit int) *Store {
	...
	return &Store{clock: time.Now, items: map[string]model.Request{}, limit: historyLimit}
}

func (store *Store) Put(value model.Request) (model.Request, error) {
...
func (store *Store) Create(value model.Request) (model.Request, error) {
	store.mu.RLock()
	_, exists := store.items[value.ID]
	store.mu.RUnlock()
	if exists {
		return model.Request{}, fmt.Errorf("request %q already exists", value.ID)
	}
	return store.Put(value)
}

func (store *Store) Get(id string) (model.Request, bool) {
	store.mu.RLock()
...
func (store *Store) Delete(id string) (model.Request, error) {
	store.mu.Lock()
	defer store.mu.Unlock()
	value, exists := store.items[id]
	if !exists {
		return model.Request{}, fmt.Errorf("request %q does not exist", id)
	}
...
func (store *Store) List() []model.Request {
	store.mu.RLock()
	defer store.mu.RUnlock()
	result := make([]model.Request, 0, len(store.items))
...
func (store *Store) FindByName(text string) []model.Request {
...
func (store *Store) ReplaceAll(values []model.Request) error {
	replacement := make(map[string]model.Request, len(values))
	now := store.clock()
	for _, value := range values {
		...
		if _, duplicate := replacement[value.ID]; duplicate {
			return fmt.Errorf("duplicate request id %q", value.ID)
		}
